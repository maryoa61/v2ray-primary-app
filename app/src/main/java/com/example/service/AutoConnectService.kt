...
class AutoConnectService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (!repository.getRuntimeSettings().autoConnectEnabled) { stopSelf(); return START_NOT_STICKY }
        ...
        pingLoopJob = serviceScope.launch {
            ...
            while (isActive && isRunning) {
                try {
                    ...
                    servers.forEach { repository.testServerPing(it.id) }
                    val updated = repository.allServers.first()
                    val current = repository.getSelectedServer()
                    val candidate = updated.filter { (it.ping ?: -1) > 0 }.minByOrNull { it.ping!! }
                    if (current != null && candidate != null && candidate.id != current.id) {
                        val healthy = (current.ping ?: -1) > 0
                        failures = if (healthy) 0 else failures + 1
                        val gap = (current.ping ?: Int.MAX_VALUE) - (candidate.ping ?: Int.MAX_VALUE)
                        if (!healthy && failures >= HEALTH_FAILURES_REQUIRED && gap >= LATENCY_SWITCH_THRESHOLD_MS) {
                            VpnCoreManager.activeVpnCoreManager?.switchServer(candidate) ?: repository.selectServer(candidate.id)
                        }
                    } else failures = 0
                } catch (e: Exception) { repository.log("AUTO-SELECTOR", "ERROR", "Background ping evaluation error: ${e.localizedMessage}") }
                delay(20_000)
            }
        }
    }
    override fun onDestroy() {
        isRunning = false; pingLoopJob?.cancel(); serviceJob.cancel(); _isServiceActive.value = false; super.onDestroy()
    }
}
